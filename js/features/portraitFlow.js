...
export async function pickCropStorePortrait(opts) {
  ...
  try {
    if (typeof setStatus === "function") setStatus("Saving image...");
    ...
    const cropped = await cropImageModal(file, { aspect, outSize, mime, quality });
    ...
    if (typeof setStatus === "function") setStatus("Could not save image. Consider exporting a backup.");
