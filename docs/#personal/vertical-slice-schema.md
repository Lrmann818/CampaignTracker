...
_Status: **RATIFIED 2026-04-22.** All open design questions resolved. Project-of-record rules absorbed into `docs/reference/content-registry-plan.md` and `AGENTS.md` — see "Absorption Checklist" at the end of this document._

_This file is a personal design record. It is the **why**. The reference docs and `AGENTS.md` are the **what**. If they conflict, the reference docs win and this file should be updated to reflect the discrepancy._
...
It exists primarily as a **design record** — a "why we built it this way" reference for future-me and any future contributors. The actual project rules live in `docs/reference/` and `AGENTS.md`; this file is the reasoning trail.
...
- `docs/LORE_LEDGER_BUILDER_PLAN.md` — phased rollout plan
...
The decisions in this document need to be reflected in tracked project files for them to be authoritative. **This file (`vertical-slice-schema.md`) is in `docs/design/` and is not pushed to GitHub** — it serves as a personal design record. The actual project rules need to land in the following files:

### `docs/reference/content-registry-plan.md` — needs to add

- [ ] Section: "Build-Time Choices Schema" — define the `choice` shape (id, kind, count, from, source) with field descriptions
- [ ] Section: "Choice `from` types" — enumerate `any`, `list with options`, `list with source`
- [ ] Section: "The `kind` vocabulary" — list current values (language, ancestry, skill, cantrip), with rule that adding new ones requires a docs update + referential integrity test update
- [ ] Section: "Trait fields" — clarify that traits are purely descriptive; `derivedFrom` is the only allowed pointer-style field; `choiceRef` is explicitly NOT a field
- [ ] Section: "Choice placement" — state that choices live inline on the parent entry (race/class/background/subclass)
- [ ] Section: "ID uniqueness" — bare ids, must be globally unique across all `game-data/srd/*.json` content files
- [ ] Section: "Referential integrity" — describe the test's purpose and what it validates
- [ ] Add `game-data/srd/draconic-ancestries.json` to the planned registry files list
- [ ] Add example record schemas for ancestry entries (with structured `breathWeapon` field)

### `AGENTS.md` — needs to add or update

- [ ] Adapter pipeline section: note that `racesAdapter.js` now populates `choices` field; note new `draconicAncestriesAdapter.js` and `traitsAdapter.js`
- [ ] Rule: traits never carry `choiceRef`; the relationship between a trait and a choice flows through the parent race/class entry
- [ ] Rule: the `kind` vocabulary in choices is a closed set — adding new kinds requires docs update
- [ ] Rule: anchor-test mechanics, never text
- [ ] Rule: ids are globally unique across all `game-data/srd/*.json`
- [ ] Reference to the referential integrity test as a quality gate

### `docs/reference/srd-licensing-notes.md` — likely unchanged

The licensing posture is unaffected by this design. No changes expected unless the description-text policy needs sharpening (already mostly covered).

### Once absorbed

When the absorption work is complete, this design doc stays in `docs/design/` as a personal record of how the decisions were reached. The reference docs become the single source of truth for what the rules *are*; this file remains the source of truth for *why they are what they are*.
