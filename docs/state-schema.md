...
- Step 3 Phase 2A does not persist `abilityMethod`. Ability-score entry method (manual, standard array, point buy, roll) is wizard-local draft state — only `build.abilities.base` scores are written to the persisted build on Finish. The `abilityMethod` field previously documented here was a forward-looking placeholder; it was removed when the builder plan tightened the intent. Phase 2B will decide whether entry method warrants persistence once non-manual methods are wired up.
