...
/************************ Party helpers (global so handlers never lose scope) ***********************/
function updateParty(id, patch, rerender = true) {
  const arr = state?.tracker?.party;
  ...
  if (rerender && typeof window.renderPartyCards === "function") window.renderPartyCards();
}
...
async function deleteParty(id) {
  ...
  state.tracker.party = arr.filter(x => x.id !== id);

  setStatus("Saving...");
  saveAll();

  if (typeof window.renderPartyCards === "function") window.renderPartyCards();
}


/************************ Party image picker (global) ***********************/
// Needed because party cards call pickPartyImage(...) from click handlers.
function pickPartyImage(id) {
  // Lazily wire the picker once
  if (!window.__partyImagePickerEl) {
    ...
    window.__partyImagePickerEl = inp;
    window.__partyPendingPartyId = null;

    inp.addEventListener("change", async () => {
      const file = inp.files?.[0];
      const pendingId = window.__partyPendingPartyId;
      if (!file || !pendingId) return;

      const m = (state.tracker.party || []).find(x => x.id === pendingId);
      if (!m) return;
      ...
      updateParty(pendingId, { imgBlobId: blobId });
      window.__partyPendingPartyId = null;
    });
  }

  window.__partyPendingPartyId = id;
  window.__partyImagePickerEl.value = "";
  window.__partyImagePickerEl.click();
}
...
function setupCalculator() {
  ...
  // Keep on-screen if viewport changes (matches your other dropdowns)
  window.addEventListener("resize", () => {
    if (!menu.hidden) positionMenuOnScreen(menu, btn, { preferRight: true });
  });
...
function setupDiceRoller() {
  ...
  // resize reposition
  window.addEventListener("resize", () => {
    if (!menu.hidden && typeof positionMenuOnScreen === "function") {
      positionMenuOnScreen(menu, btn, { preferRight: true });
    }
  });
...
function initNpcsUI() {
  ...
  function renderNpcCards() {
    const prevScroll = cardsEl.scrollTop; // ✅ keep scroll position
    ...
    if (list.length === 0) {
      ...
      cardsEl.scrollTop = prevScroll; // ✅ restore even on empty
      return;
    }
    ...
    cardsEl.scrollTop = prevScroll; // ✅ restore after DOM rebuild
  }
...
function initPartyUI() {
  ...
  // Bind search (NPC-style: filters cards) + Sessions-style: also filters which tabs show
  searchEl.value = state.tracker.partySearch;
  searchEl.addEventListener("input", () => {
    state.tracker.partySearch = searchEl.value;
    setStatus("Saving...");
    saveAll();
    renderPartyTabs();   // ✅ sessions-like: tabs react to search
    renderPartyCards();  // ✅ npc-like: cards react to search
  });
  ...
  function renderPartyTabs() {
    ...
    // If search would hide the active tab, keep it visible (like sessions keeping active notes visible)
  ...
  addSectionBtn.addEventListener("click", () => {
    ...
    const proposed = prompt("New section name:", `Section ${nextNum}`);
  ...
  renameSectionBtn.addEventListener("click", () => {
    ...
    const proposed = prompt("Rename section to:", sec.name || "");
    if (proposed === null) return;
  ...
  deleteSectionBtn.addEventListener("click", () => {
    ...
    if (!confirm(`Delete section "${sec.name}"? Party members in it will be moved to the first section.`)) return;
  ...
  function renderPartyCards() {
    ...
    if (list.length === 0) {
      ...
      cardsEl.appendChild(empty);
      cardsEl.scrollTop = prevScroll; // ✅ restore
      return;
    }
  ...
  // Expose for global updateParty/deleteParty helpers
  window.renderPartyCards = renderPartyCards;
  ...
  function renderPartyCard(m) {
    ...
    // (…keep your existing Class / HP / Status / Notes blocks exactly as-is…)
    // NOTE: I’m not repeating those here to keep this readable.
    // Leave everything from your current renderPartyCard between:
    //   const classRow = … through notesBlock …
    // untouched.

    // ✅ Paste your existing classRow/hpRow/statusRow/notesBlock code here:
    // ---- START existing blocks ----
    ...
    sectionSelect.className = "mapSelect"; // reusing your styled select look
    ...
    sectionSelect.addEventListener("change", () => {
      updateParty(m.id, { sectionId: sectionSelect.value }, true);
      renderPartyTabs(); // so tab filtering (search) stays accurate
    });
  ...
  // --- keep the rest of your existing initPartyUI code BELOW this point unchanged ---
  // (hidden picker, image crop, updateParty, deleteParty, etc.)
