...
function onPointerMove(e) {
  ...
  if (gestureMode === "panzoom" && activePointers.size >= 2) {
    ...
    // Pinch zoom (dead-zone to prevent "accidental zoom" while scrolling)
    const distNow = distance(pts[0], pts[1]);
    const distDelta = distNow - (pinchStart?.dist || 0);

    const PINCH_DEADZONE_PX = 20; // feel knob (bigger = less zoom jitter)
    if (Math.abs(distDelta) > PINCH_DEADZONE_PX && (pinchStart?.dist || 0) > 0) {
